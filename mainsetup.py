...
twitch_miner = TwitchChannelPointsMiner(
    username="azlankhid",
    password="hidekikaranari",           # If no password will be provided, the script will ask interactively
    ...
    logger_settings=LoggerSettings(
        ...
        telegram=Telegram(                                                          # You can omit or set to None if you don't want to receive updates on Telegram
            ...
            token="6306764741:AAE3sPCyyjKzAQjJJT2Nef3PEltdAS1kfPU",                          # Telegram API token @BotFather
            events=[Events.STREAMER_ONLINE, Events.STREAMER_OFFLINE,
                    Events.BET_LOSE, Events.CHAT_MENTION, Events.GAIN_FOR_CLAIM, Events.GAIN_FOR_WATCH, Events.MOMENT_CLAIM, Events.JOIN_RAID],                          # Only these events will be sent to the chat
            disable_notification=True,                                              # Revoke the notification (sound/vibration)
        ),
        discord=Discord(
            webhook_api="https://discord.com/api/webhooks/1195135388721893487/-vYd8CwJhjWg4c1D4_3IryAzOMeo-7B9WN3aqvEpbNHe6Hw0vGkzek0Fw0yxKuIpxmx0",  # Discord Webhook URL
            events=[Events.STREAMER_ONLINE, Events.STREAMER_OFFLINE,
                    Events.BET_LOSE, Events.CHAT_MENTION, Events.GAIN_FOR_CLAIM, Events.GAIN_FOR_WATCH, Events.MOMENT_CLAIM, Events.JOIN_RAID],                                  # Only these events will be sent to the chat
        ),
        webhook=Webhook(
...
twitch_miner.mine(
    [
        ...
        Streamer("garitdewana", settings=StreamerSettings(make_predictions=False , follow_raid=True  , claim_drops=False ,                     bet=BetSettings(strategy=Strategy.PERCENTAGE , percentage=5 , stealth_mode=False, percentage_gap=20 , max_points=1234  , filter_condition=FilterCondition(by=OutcomeKeys.TOTAL_POINTS,     where=Condition.GTE, value=250 ) ) )),
        Streamer("sveyfr", settings=StreamerSettings(make_predictions=True  , follow_raid=False ,                     watch_streak=True , bet=BetSettings(strategy=Strategy.SMART      , percentage=5 , stealth_mode=False, percentage_gap=30 , max_points=50000 , filter_condition=FilterCondition(by=OutcomeKeys.ODDS,             where=Condition.LT,  value=300 ) ) ))
